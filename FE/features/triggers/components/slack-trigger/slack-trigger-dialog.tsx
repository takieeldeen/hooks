...
import z from "zod";
import { SlackTriggerNodeData } from "./slack-trigger-node";
...
interface Props {
  ...
  nodeData: SlackTriggerNodeData;
  nodeId: string;
}

function SlackTriggerDialog({ open, onOpenChange, nodeData, nodeId }: Props) {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="left" className="dark:bg-neutral-900 overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>
            Configure the Slack account, server and channel to listen to events.
          </SheetDescription>
        </SheetHeader>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="slackTrigger" {...field} />
                    </FormControl>
                    <FormDescription>
                      Use this name to reference the result in other nodes:{" "}
                      {`{{${field.value || "slackTrigger"}.message}}`}
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
