...
export function deleteFetcher<T = unknown>(
  url: string | [string, AxiosRequestConfig],
) {
  return async (data?: any) => {
    const URL = Array.isArray(url) ? url?.[0] : url;
...
export function dummyFetcher<T>(
  ...
  const page = (URL?.[1] as any)?.params?.page
    ? (URL?.[1] as any).params.page - 1
    : 0;
  ...
  const entityKey = paginated
    ? Object?.keys(data as any)?.find((key: string) =>
  ...
  const filteredTotalNoOfRows = entityKey
    ? (data as any)?.[entityKey]?.filter(
        (el: any) =>
          el?.nameAr?.includes(name) ||
          el?.nameEn?.includes(name) ||
          el?.[nameAttr]?.includes(name),
      ).length
    : 0;
  ...
  const paginatedArray =
    paginated && entityKey
      ? filteredArray.slice?.(page * size, page * size + size)
      : [];
  ...
  paginatedObject.results =
    name && name !== "" ? filteredTotalNoOfRows : paginatedObject.results;
